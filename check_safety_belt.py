...
def is_belt_worn_correctly(belt_boxes, keypoints):
    ...
    Belt should NOT be under both knees. Keypoints format (COCO): [nose, left_eye, right_eye, left_ear, right_ear,
...
def process_image(safety_belt_model, pose_model, img_path, device):
    ...
    cv2.putText(img, f"Belt Status: {status}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)
...
def parse_opt():
    ...
    parser.add_argument(
        "--safety-belt-weights", type=str, default="safety_belt.pt", help="safety belt detection model path"
    )
